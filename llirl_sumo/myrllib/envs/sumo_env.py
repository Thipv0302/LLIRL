...
class SUMOEnv(gym.Env):
    ...
    """
    def __init__(self, sumo_config_path, max_steps = 4090, yellow_time=3, green_time_min=10, green_time_max=60, use_gui=False):
        ...
        self.observation_space = spaces.Box(
            low=0.0, 
            high=np.inf, 
            shape=(self.num_lanes * 3,),  # queue_length, waiting_time, vehicle_count per lane
            dtype=np.float32
        )
    ...
    def _get_reward(self):
        ...
        if lane_count > 0:
            ...
            reward -= avg_waiting * 0.05  # Tăng từ 0.05 → 0.1
            
            # 2. Penalty for queue (tăng weight, thêm exponential penalty)
            reward -= avg_queue * 0.5  # Tăng từ 0.5 → 0.8
            if avg_queue > 5.0:
            ...
            if avg_speed > 0:
                speed_reward = (avg_speed / 20.0) * 0.3  # Tăng từ 0.3 → 0.5
                reward += speed_reward
            
            # 5. BONUS lớn hơn cho low congestion
            if avg_queue < 2.0 and avg_speed > 5.0:
                reward += 1.0  # Tăng từ 1.0 → 5.0
    ...
    def step(self, action):
        ...
        norm_actions = np.tanh(action)  
        action = self.green_time_min + (norm_actions + 1.0) * 0.5 * (self.green_time_max - self.green_time_min)
        ...
        # ---- RL CHỈ ĐƯỢC CHỌN THỜI GIAN CHO PHASE HIỆN TẠI ----

        # LƯU phase cũ trước khi simulationStep
        old_phase = self.last_phase

        # BƯỚC 1: đọc phase hiện tại từ SUMO (đây là pha cũ)
        current_phase = traci.trafficlight.getPhase(self.tl_id)
        #check phase
        # print(f"[PHASE DEBUG] step={self.step_count}, phase={current_phase}")

        # BƯỚC 2: chạy simulation
        traci.simulationStep()

        # BƯỚC 3: đọc phase mới sau khi SUMO cập nhật
        new_phase = traci.trafficlight.getPhase(self.tl_id)
        # #check phase
        # print(f"[PHASE DEBUG AFTER STEP] step={self.step_count}, new_phase={new_phase}")

        # TÍNH duration tương ứng với phase mới
        # ===== NEW ACTION LOGIC FOR shape=(1,) =====
        # Action đã là 1 số duy nhất → dùng chung cho cả phase 0 và phase 2
        phase_time = float(action[0])   # 1 action duy nhất

        if new_phase in (0, 2):  
            phase_duration = int(np.clip(phase_time,
                                        self.green_time_min,
                                        self.green_time_max))
        else:
            # Yellow phases giữ nguyên 3 giây
            phase_duration = 3
            #check phase
            # print(f"[YELLOW PHASE] phase={new_phase}, keep duration=3")

        # BƯỚC 4: Nếu phase mới KHÁC phase cũ → apply duration ngay lập tức
        if new_phase in (0, 2) and new_phase != old_phase:
            try:
                traci.trafficlight.setPhaseDuration(self.tl_id, phase_duration)
                #check phase
                # print(f"[DURATION APPLIED] phase={new_phase}, duration={phase_duration}")
            except Exception as e:
                print("[ERROR APPLYING DURATION]", e)

        # Yellow phases → skip
        elif new_phase in (1, 3):
            pass
        ...
        while phase_steps < phase_duration and self.step_count < self.max_steps:
            try:
                ...
                self.step_count += 1
                phase_steps += 1
                phase_now = traci.trafficlight.getPhase(self.tl_id)
                #check phase
                # print(f"[PHASE DEBUG] step={self.step_count}, phase={phase_now}")
                
                # Collect reward
                try:
                    step_reward = self._get_reward()
                    total_reward += step_reward
                    # print(f"[TEST REWARD] step={self.step_count}, reward={step_reward}, total={total_reward}")
                except (traci.exceptions.FatalTraCIError, traci.exceptions.TraCIException, Exception) as e:
     ...
     # --- collect metrics for evaluation ---
        ...
        if self.step_count >= 4090:
            truncated = True     # Dừng vì hết thời gian
