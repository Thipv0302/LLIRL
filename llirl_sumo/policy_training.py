...
from myrllib.envs_ddqn import sumo_env
import gym
gym.register(
...
def inner_train(policy, learner, period=0, track_metrics=True):
    ...
    for idx in tqdm(range(args.num_iter)):
        ...
        loss = None
        try:
            loss_val, grad_norm = learner.step(episodes, clip=True)
        except TypeError:
            # Trường hợp learner.step không return gì
            learner.step(episodes, clip=True)

        print(f"[DEBUG] Period={period}, Iter={idx}, Loss={loss_val}, GradNorm={grad_norm}")
...
training_metrics['losses'].append(init_metrics.get('losses', []))
...
try:
    for period in range(1, args.num_periods):
        ...
        training_metrics['losses'].append(period_metrics.get('losses', []))
